...
export default function LearnScreen() {
  // State variables
  const [activeTab, setActiveTab] = useState<'chat' | 'carbon' | 'events'>('chat');
    ...
    try {
      ...
      if (data.success) {
        ...
        const welcomeMessage: Message = {
          ...
          text: "Welcome to Soular's Learning Assistant! I can help you calculate your carbon footprint, suggest green events, or answer questions about climate change. What would you like to learn today?",
          sender: 'ai',
          timestamp: new Date()
        };
  ...
  const sendMessage = async (message: string) => {
    ...
    try {
      ...
      } else if (message.toLowerCase().includes('event') || message.toLowerCase().includes('events')) {
        ...
        if (jsonStartIndex >= 0 && jsonEndIndex > jsonStartIndex) {
          ...
          const footprintData: CarbonFootprintResult = {
            ...
            tips: Array.isArray(parsedData.tips)
              ? parsedData.tips
              : ['Reduce energy consumption', 'Use public transportation', 'Eat less meat']
          };
          ...
          if (Array.isArray(footprintData.breakdown)) {
            breakdownText = footprintData.breakdown.map(item =>
...
handleTabChange('events'); // Ensures the gradient and animations are updated
  ...
  const renderMessage = (message: Message, index: number) => {
    ...
    return (
      ...
      <View style={{ maxWidth: '80%', flexDirection: 'row', justifyContent: 'center', marginBottom: 10, alignItems: 'center' }}>
        {/* Profile Image */}
      ...
      >
        <View style={{
          ...
          <View style={{
            ...
            <Text style={{ fontSize: 18, color: 'white', marginRight: 10, marginLeft: 10 }}>
              {carbonData.unit}
            </Text>
            </View>
          </View>

          <Text style={{ fontSize: 18, fontWeight: 'bold', color: '#212529', marginBottom: 8 }}>
            Breakdown
          </Text>

          {Array.isArray(carbonData.breakdown) && carbonData.breakdown.map((item, index) => (
        ...
        <View style={{
          ...
          <Text style={{ fontSize: 18, fontWeight: 'bold', color: '#212529', marginBottom: 12 }}>
            Tips to Reduce Your Footprint
          </Text>

          {Array.isArray(carbonData.tips) && carbonData.tips.map((tip, index) => (
            <View key={index} style={{
              flexDirection: 'row',
              marginBottom: 12,
              ...
              <Text style={{ flex: 1, fontSize: 16, color: '#495057', lineHeight: 24 }}>
                {tip}
              </Text>
            </View>
          ))}
          ...
          >
            ...
            <View style={{ padding: 20}}>
              ...
              <View style={{ flexDirection: 'row', marginBottom: 8, alignItems: 'center' }}>
                <Ionicons name="calendar-outline" size={16} color="#007AFF" style={{ marginRight: 10 }} />
                <Text style={{ fontSize: 14, color: '#6C757D', marginRight: 12 }}>{event.date}</Text>


              </View>
              <View style={{ flexDirection: 'row', marginBottom: 20, alignItems: 'center' }}>

                              <Ionicons name="location-outline" size={16} color="#007AFF" style={{ marginRight: 10 }} />
                              <Text style={{ fontSize: 14, color: '#6C757D' }}>{event.location}</Text>
                            </View>

              <Text style={{ fontSize: 16, color: '#495057', marginBottom: 8, lineHeight: 22 }}>
                {event.description}
              </Text>
        ...
        {activeTab === 'chat' && (
          <View style={{ flex: 1, marginBottom: 50 }}>
            ...
            >
              ...
              {isLoading && (
                <View style={{
                  ...
                  <ActivityIndicator size="small" color="#1aea9f" style={{ marginRight: 8 }} />
            ...
            <View style={{
              ...
              />
              <TouchableOpacity
                onPress={() => inputMessage.trim() && sendMessage(inputMessage.trim())}
              ...
              >
                <Ionicons name="send" size={20} color={inputMessage.trim() ? 'white' : '#ADB5BD'} />
              </TouchableOpacity>
            </View>
          </View>
        )}
